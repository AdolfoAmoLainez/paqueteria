...
class PHPMailer
{
    ...
    private function mailPassthru($to, $subject, $body, $header, $params)
    {
        ...
        if (ini_get('safe_mode') || !($this->UseSendmailOptions))  $result = @mail($to, $subject, $body, $header);
    ...
    public function setFrom($address, $name = '', $auto = true)
    {
        ...
        $this->FromName = $name;
        if ($auto) {
            if (empty($this->Sender))   $this->Sender = $address;
        }
        return true;
    }
    ...
    protected function smtpSend($header, $body)
    {
        ...
        if (!$this->smtpConnect())  throw new phpmailerException($this->lang('smtp_connect_failed'), self::STOP_CRITICAL);
    ...
    public function setLanguage($langcode = 'en', $lang_path = '')
    {
        ...
        if ($langcode != 'en') { // There is no English translation file
            ...
            if (!is_readable($lang_file))  $foundlang = false;
    ...
    public function getMailMIME()
    {
        ...
        if ($this->Encoding != '7bit') {
            ...
            if ($ismultipart) {
                if ($this->Encoding == '8bit')  $result .= $this->headerLine('Content-Transfer-Encoding', '8bit');
    ...
    protected function encodeFile($path, $encoding = 'base64')
    {
        try {
            ...
            if ($magic_quotes) {
                if (version_compare(PHP_VERSION, '5.3.0', '<')) set_magic_quotes_runtime(false);
            ...
            if ($magic_quotes) {
                ...
                else  ini_set('magic_quotes_runtime', ($magic_quotes?'1':'0'));
    ...
    public function addStringEmbeddedImage(
        ...
        if ($type == '')  $type = self::filenameToType($name);
    ...
    public function set($name, $value = '')
    {
        try {
            ...
            else throw new phpmailerException($this->lang('variable_set') . $name, self::STOP_CRITICAL);
            
        } catch (Exception $exc) {
            $this->setError($exc->getMessage());
            if ($exc->getCode() == self::STOP_CRITICAL) return false;
        }
        return true;
    }
    ...
    {
        if (!defined('PKCS7_TEXT')) {
            if ($this->exceptions)  throw new phpmailerException($this->lang('signing') . ' OpenSSL extension missing.');
            return '';
        }
    ...
    public function DKIM_Add($headers_line, $subject, $body)
    {
        ...
        foreach ($headers as $header) {
            if (strpos($header, 'From:') === 0) {
                ...
                if ($current && strpos($header, ' =?') === 0)  $current .= $header;
                else $current = '';
            }
        }
